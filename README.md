...
[![tests](https://img.shields.io/badge/tests-344%20passed-brightgreen.svg)](#-评测)
...
- **`admin`**（`admin/`）—— 控制面。登录走**账号密码 + 服务端会话**，不是 bearer token：浏览器地址栏设不了 header，所以 token 方案在浏览器里**只能**退化成 `/admin?key=…`，而查询串会进访问日志、每条外链的 `Referer` 和浏览历史。密码 **scrypt** 加盐哈希（标准库，零依赖）；会话是**服务端行**而不是签名 Cookie——自带声明的签名 Cookie 在过期前一直有效，撤销不了，而「立刻把这个人踢下线」正是控制面需要的动作。策略在**被读取之前**就已合成为生效值（`registry/effective.py`），所以既有 11 个策略读取点一行不用改，**忘记某条路径在结构上不可能**。热生效一律原地替换 `State.policies[tenant]`，**从不 `cache_clear()`**——清缓存会连带丢掉 `RoutingLog`，而那正是控制台展示 G1 否决的唯一来源。
...
# 5. 建控制面账号（不经过网络，密码不进任何文件）
make admin-add USER=kevin              # 提示输入密码，默认 rw
make admin-add USER=li ROLE=ro         # 只读管理员

# 6. 启动网关 + 控制台 + 控制面（make run 会把 .env 载进进程环境）
...
- ✅ **P4d 控制面登录** —— 账号密码 + 服务端会话取代共享 key。旧方案在浏览器里**只能**退化成 `/admin?key=…`（地址栏设不了 header），而查询串会进访问日志、`Referer` 和浏览历史。密码 scrypt 加盐、会话可撤销、按账号锁定（一次针对某个用户名的爆破不会把其他管理员一起锁在门外）。账号只能从命令行建：`.env` 里的引导密码是一份会被复制和提交的明文凭据，而能自己发第一个管理员的控制面有一个谁都能当管理员的窗口
...
- ✅ **P4e 控制面改版** —— 六个菜单（总览 / 业务 / 密钥 / 权限 / 日志 / 账户），**每个操作都从它所属的那一行打开**，业务线预填且不可改：先选行、再去页底表单里把业务线重选一遍，是两次独立选错的机会，而两次都会写进生效策略。界面**只说业务话**（`cross_tenant_read` → 跨业务线查账、nano-USD → 美元、diff → 待落地配置），原始字段名只活在那段要交给工程侧落盘的配置里——翻译它就等于交付一份跑不通的 YAML。术语表是**一张可审阅的表**（`static/terms.js`），并且有测试遍历 `api.py` 里每一处 `cp.record(...)`，少一条中文就红。总览改成真 dashboard：常驻上游横幅 + 今日水位 + **九类异常告警三态呈现**（没测过的画成灰色「暂无数据」，不是绿色的 0），且**两个口径分开标注**——账本推导的标「今日」，用的是网关判 429 的同一个 `day_start`；路由否决标「本次启动以来」，因为那是进程内存、重启即失。补齐三个本该有的动作：改自己的密码（顺手撤销自己其他会话）、恢复被停用的账号、解冻登录失败锁定；**唯独不做「重置别人的密码」**——能把同事的密码改成自己知道的值，大幅提额的第二人签字就不再是签字
